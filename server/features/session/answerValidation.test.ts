...
describe('checkAnswer — single translation', () => {
  it('returns true when the answer matches the only translation', () => {
    ...
    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['table'])).toBe(true)
  })
  ...
  it('returns true when hyphen/space normalisation makes it match', () => {
    const entry = makeEntry({ target: ['well-known'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['well known'])).toBe(true)
  })

  it('returns false when the answer does not match', () => {
    const entry = makeEntry({ target: ['table'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['chair'])).toBe(false)
  })

  it('returns false for an empty answers array', () => {
    const entry = makeEntry({ target: ['table'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', [])).toBe(false)
  })

  it('uses source translations for TARGET_TO_SOURCE direction', () => {
    const entry = makeEntry({ source: 'Tisch', target: ['table'] })

    expect(checkAnswer(entry, 'TARGET_TO_SOURCE', ['Tisch'])).toBe(true)
    expect(checkAnswer(entry, 'TARGET_TO_SOURCE', ['table'])).toBe(false)
  })
})
...
describe('checkAnswer — multiple translations', () => {
  it('returns true when two correct answers are provided', () => {
    ...
    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['bicycle', 'bike'])).toBe(true)
  })

  it('returns true regardless of answer order', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['bike', 'bicycle'])).toBe(true)
  })

  it('returns false when only one correct answer is given', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['bicycle'])).toBe(false)
  })

  it('returns false when the same translation is repeated twice', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['bicycle', 'bicycle'])).toBe(false)
  })

  it('returns false when one answer is wrong', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['bicycle', 'wheel'])).toBe(false)
  })

  it('requires exactly 2 even when there are 3 translations', () => {
    const entry = makeEntry({ target: ['a', 'b', 'c'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['a', 'b'])).toBe(true)
    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['a'])).toBe(false)
  })
})
...
describe('checkAnswerDetailed', () => {
  it('returns correct=true and matchedCount=1 for a fully correct single-translation answer', () => {
    const entry = makeEntry({ target: ['table'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['table'])
  ...
  it('returns correct=true and matchedCount=2 for a fully correct two-translation answer', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['bicycle', 'bike'])
  ...
  it('returns correct=false and matchedCount=1 when only one of two required answers is correct', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['bicycle', 'wrong'])
  ...
  it('returns correct=false and matchedCount=0 when no answers match', () => {
    const entry = makeEntry({ target: ['bicycle', 'bike'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['car', 'truck'])
...
describe('checkAnswerDetailed — typo detection', () => {
  it('accepts a one-character transposition as correct and records a typo', () => {
    ...
    const entry2 = makeEntry({ target: ['recieve'] }) // intentionally misspelled as target

    const result = checkAnswerDetailed(entry2, 'SOURCE_TO_TARGET', ['receive'])
  ...
  it('accepts an answer with distance 1 on a 7-character word (short-word rule: < 8 chars)', () => {
    ...
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['machone'])
  ...
  it('accepts an answer with distance 1 on a short word (< 8 chars)', () => {
    ...
    const entry = makeEntry({ target: ['table'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['tble'])
  ...
  it('rejects an answer with distance 2 on a short word (< 8 chars)', () => {
    ...
    const entry = makeEntry({ target: ['table'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['tbl'])
  ...
  it('accepts an answer with distance 2 on a 15-character word (ratio ≈ 13%)', () => {
    ...
    const entry = makeEntry({ target: ['congratulations'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['congratulatinos'])
  ...
  it('rejects a completely wrong answer (large distance)', () => {
    const entry = makeEntry({ target: ['table'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['elephant'])
  ...
  it('records typos for each individually typo-matched answer in a two-translation word', () => {
    const entry = makeEntry({ target: ['machine', 'apparatus'] })

    // 'machone' matches 'machine' with 1 edit (7 chars, ratio ≈ 0.14)
    // 'apparatos' matches 'apparatus' with 1 edit (9 chars, ratio ≈ 0.11)
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['machone', 'apparatos'])
  ...
  it('does not record a typo when the match is exact', () => {
    const entry = makeEntry({ target: ['machine', 'apparatus'] })
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['machine', 'apparatus'])
  ...
  it('records only the typo match when one answer is exact and one is a typo', () => {
    ...
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['machine', 'apparatos'])
...
describe('checkAnswerDetailed — collapsed match', () => {
  it('accepts "softdrink" for "soft drink" and records a typo', () => {
    const entry = makeEntry({ target: ['soft drink'] })

    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['softdrink'])
  ...
  it('accepts "wellknown" for "well-known" and records a typo', () => {
    ...
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['wellknown'])
  ...
  it('does not record a typo when the user writes "well known" for "well-known" (exact after normalisation)', () => {
    ...
    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['well known'])
  ...
  it('collapsed match takes priority over Levenshtein for spacing errors', () => {
    const entry = makeEntry({ target: ['swimming pool'] })

    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['swimmingpool'])
...
describe('checkAnswer — "to " prefix stripping', () => {
  it('accepts user input "stand up" when DB stores "to stand up"', () => {
    const entry = makeEntry({ target: ['to stand up'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['stand up'])).toBe(true)
  })

  it('accepts user input "to stand up" when DB stores "stand up"', () => {
    const entry = makeEntry({ target: ['stand up'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['to stand up'])).toBe(true)
  })
  ...
  it('does not strip "to" when it is not followed by a space (e.g. "tomorrow")', () => {
    const entry = makeEntry({ target: ['buy'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['tomorrow'])).toBe(false)
  })

  it('does not count a "to " prefix match as a typo', () => {
    const entry = makeEntry({ target: ['to stand up'] })

    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['stand up'])
...
describe('checkAnswer — deduplication of equivalent translations', () => {
  it('accepts a single answer when "sleep" and "to sleep" are both stored', () => {
    const entry = makeEntry({ target: ['sleep', 'to sleep'] })

    expect(checkAnswer(entry, 'SOURCE_TO_TARGET', ['sleep'])).toBe(true)
  })

  it('requires only one answer after deduplication reduces two entries to one', () => {
    const entry = makeEntry({ target: ['sleep', 'to sleep'] })

    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['sleep'])
  ...
  it('still requires two answers when translations are genuinely distinct after deduplication', () => {
    const entry = makeEntry({ target: ['sleep', 'to roost'] })
  ...
  it('deduplicates three translations where two are equivalent', () => {
    const entry = makeEntry({ target: ['sleep', 'to sleep', 'to roost'] })

    const result = checkAnswerDetailed(entry, 'SOURCE_TO_TARGET', ['sleep', 'roost'])
